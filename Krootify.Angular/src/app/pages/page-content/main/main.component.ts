...
export class MainComponent implements OnInit {
  ...
  constructor(private songService: SongService, private playListService: PlaylistService, private elementRef: ElementRef) {}
  ...
  getCategories(){
    this.songService.getSongCategories().subscribe(response => {
      this.categories = [{id: 0, categoryName: 'All categories'}, ...response];
    }, error => {
      console.log(error);
    })
  }
  ...
  addSongToPlayList(songId: number, playListId: number){
    ...
    this.playListService.addSongToPlayList(songPlayList).subscribe(response => {
      this.getSongs();
      this.getPlayLists();
    }, error => {
      console.log(error);
  ...
  deleteSongFromPlayList(songId: number, playListId: number){
    this.playListService.deleteSongFromPlayList(songId, playListId).subscribe(response => {
      this.getSongs();
      this.getPlayLists();
    }, error => {
      console.log(error);
